...
Сейчас этот проект неактивен, поэтому я написал свой форк [LinkedIn-AI-Job-Applier-Ultimate](https://github.com/beatwad/LinkedIn-AI-Job-Applier-Ultimate) для поиска работы на LinkedIn, сильно модифицированный и улучшенный. 
Если вас интересует поиск работы именно на LinkedIn, советую приглядеться к нему.
...
Отличием XX_Auto_Jobs_Applier от базовой версии является также возможность написания резюме с учетом информации о пользователе и особенности
...
4. **Мониторинг работы приложения**
   ...
   - Сообщения об ошибках отправляются в раздел `Errors` [чата Telegram](https://t.me/xx_feedback)
   ...
   - Результаты работы (количество вакансий, на которые удалось откликнуться и список вакансий, на которые откликнуться не удалось) в конце работы бота отправляются по email
   - Приложение осуществляет мониторинг появления капчи, сообщает о ее появлении в раздел `Captcha` [чата Telegram](https://t.me/xx_feedback) и после того, как пользователь ответит на его сообщение своим сообщением с решением капчи, использует это решение для ответа на капчу на сайте

5. **Безопасная обработка данных**
   - Все приватные данные хранятся локально в YAML файлах

## Получение токенов для доступа к API hh.ru
...
1. **Заявка**
  ...
  Заполняем заявку на [dev.hh.ru/admin](https://dev.hh.ru/admin):
    ...
    Приложением будут пользоваться:
    Только соискатели

    Информация о создателе приложения:
    Программист с опытом работы на Go, Python и Java.

    Кто будет его использовать:
    Зарегистрированные пользователи hh.ru

    Какие задачи должно решать приложение:
    - предоставление рекомендаций по улучшению и исправлению резюме пользователя
    - помощь в создании сопроводительных писем
    - подбор наиболее релевантных вакансий

    Опишите все функциональные возможности приложения и укажите используемые методы API:

    Функциональные возможности приложения:
    1. Рекомендации по доработке резюме
    2. Анализ соответствия резюме требованиям вакансии
    3. Создание сопроводительных писем на основе вакансий и резюме
    4. Возможность отклика на вакансию через приложение

    Используемые методы API:
    1. Авторизация приложения и пользователя
    2. Получения списка резюме пользователя
    3. Просмотр резюме пользователя
    4. Публикация резюме
    5. Получение списка вакансий
    6. Получение информации о вакансии
    7. Поиск по вакансиям
    8. Отклик на вакансию

2. Периодически проверяем статус заявки. Обычно одобряют в течение нескольких дней. 

3. Далее заходим по указанной выше ссылке и видим следующее:

<img src="./assets/image_1.png">

4. Задаем Redirect Uri, любой доступный, я поставил localhost

5. Копируем значения ключей Client ID и Client Secret

6. Используем Client ID и Client Secret для создания ссылки для получения Auth Code, вот код для получения:

```python
client_id = "your_client_id" # ваш Client Id 
client_secret = "your_client_secret" # ваш Client Secret
redirect_uri = "https://localhost" # Ваш эндпоинт на домене, который будет принимать токен авторизации, рекомендую не заморачиваться и просто поставить localhost

url = f"https://hh.ru/oauth/authorize?response_type=code&client_id={client_id}&redirect_uri={redirect_uri}"

print(url)
```
7. Переходим по получивишейся ссылке и одобряем авторизацию в hh.ru под тем аккаунтом, с которого планируем откликаться, после редиректа в поле адреса увидите строку вида http://localhost/?code=auth_code; скопируйте значение auth_code 

8. Далее при помощи полученного Auth code получаем Access Token и Refresh Token, вот код для получения данных токенов:

```python
import requests

auth_code = "your_auth_code" # ваш Auth Code
...
Задаем полученные Access и Refresh токены в соответствующих полях файла `data_folder/secrets/secrets.yaml`

Если использовать приложение регулярно, то об этих токенах можно забыть, приложение будет обновлять их самостоятельно. Если же не пользоваться приложением несколько недель, то весь процесс получения токенов (за исключением регистрации самого приложения, разумеется) придется повторить заново. 
...
6. **Дополнительно (но не обязательно)**:
  ...
  - [Создайте пароль для работы приложения для отправки отчетов по email](https://support.google.com/mail/answer/185833?hl=en) (на примере Gmail) и добавьте адрес email и полученный пароль в файл `secrets.yaml`, в поля `sender_email` и `email_password` соответственно
...
- `access_token: [Токен доступа к hh API]`
- `refresh_token: [Токен обновления прав доступа к hh API]`

- `hh_login`: Логин пользователя на сайте hh.ru. Нужен для ответов на вакансии с вопросами.
- `hh_password: [Пароль пользователя от сайта hh.ru. Нужен для ответов на вакансии с вопросами.]`
...
- `user_id:`
  - ID пользователя на hh.ru, нужен для идентификации пользователя, у которого вылетела ошибка
...
2. **data_folder/output:**
    ...
    - `llm_api_calls.yaml` лог всех запросов, сделанных к LLM, и полученных на них ответов
...
3. **Запуск:**
   ...
   python main.py
   ```
   Если переменная `RESUME_MODE` установлена в значение True, то перед запуском приложение предложит выбрать, в каком стиле нужно генерировать резюме.

   При первом запуске приложение использует логин и пароль для авторизации на сайте, возможно потребуется ввод капчи. При последующих запусках этого не потребуется, так как все данные для входа сохраняются в папке `chrome_profile`.

   Затем приложение соберет все доступные ему данные о резюме пользователя из его анкеты на сайте, введет настройки поиска и начнет искать вакансии в соответствии с критериями поиска и откликаться на них.
