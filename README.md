...
4. **Установите uv (менеджер пакетов и окружений):**

   Проект использует [uv](https://docs.astral.sh/uv/). Установите его по [официальной инструкции](https://docs.astral.sh/uv/getting-started/installation/):
...
5. **Создайте окружение и установите все необходимые пакеты:**

   ```bash
   uv sync
   ```

   Эта команда сама скачает нужную версию Python (3.12), создаст виртуальное окружение в папке `.venv` и установит все зависимости из `pyproject.toml` (включая инструменты для тестов).

   Установите браузеры для Playwright (если используете не системный Chrome):
...
В частности на этапе выполнения команды `uv sync`.

Убедитесь, что uv установлен корректно (`uv --version`). uv сам подберет нужную версию Python (3.12) согласно `requires-python` в `pyproject.toml`.

Если проблема сохраняется, попробуйте пересоздать окружение: удалите папку `.venv` и выполните `uv sync` заново.
