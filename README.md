...
1. **Заявка**
  ...
  Заполнить заявку на [dev.hh.ru/admin](https://dev.hh.ru/admin):
...
2. Периодически проверять статус заявки. Обычно одобряют в течение нескольких дней. 

3. Далее заходим в админку и видим следующее:
...
auth_code = "789"  # измените на фактически полученный в п.4 код
